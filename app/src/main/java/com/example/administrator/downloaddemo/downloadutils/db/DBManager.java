...
public class DBManager {
    ...
    public void saveInfos(List<DownloadInfo> infos) {
        ...
        for (DownloadInfo info : infos) {
            db.execSQL(
                    "insert into downloadinfo(thread_id,start_pos, end_pos,compelete_size,url) "
                            + "values (?,?,?,?,?)",
                    new Object[]{info.getThreadId(), info.getStartPos(), info.getEndPos(),
    ...
    public void saveInfo(DownloadInfo info) {
        SQLiteDatabase db = helper.getWritableDatabase();
        db.execSQL(
                "insert into downloadinfo(thread_id,start_pos, end_pos,compelete_size,url) values"
                        + " (?,?,?,?,?)",
                new Object[]{info.getThreadId(), info.getStartPos(), info.getEndPos(),
                        info.getCompeleteSize(), info.getUrl()});
    }
...
//        String sql = "select count(*) from downloadinfo where url=?";
    ...
    public List<DownloadInfo> getInfos(String urlstr) {
        ...
        String sql =
                "select thread_id, start_pos, end_pos,compelete_size,url from downloadinfo where "
                        + "url=?";
    ...
    public DownloadInfo getInfo(String urlstr, int threadid) {
        ...
        Log.d("DB",
                "get db info print: " + cursor.getInt(cursor.getColumnIndex("thread_id")) + " " +
                        ...
                        cursor.getColumnIndex("end_pos")) + " " + cursor.getInt(
                        cursor.getColumnIndex("compelete_size")) + " ");
        cursor.close();
        return info;
    }
