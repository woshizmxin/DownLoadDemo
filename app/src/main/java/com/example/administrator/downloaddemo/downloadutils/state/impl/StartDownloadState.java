...
public class StartDownloadState implements DownloadState {
    ...
    @Override
    public void startDownload(final Context context, final Handler handler,
            final String downloadurl, final String filename, final int threadcount) {
        ThreadPoolsUtil.getInstance().getFixedThreadPool().execute(new Runnable() {
            @Override
            public void run() {
                int filesize = getDownloadFilesize(downloadurl);
                FileDownloader.getInstance().init(context, handler, downloadurl, filesize, filename,
                        threadcount).startDownload();
